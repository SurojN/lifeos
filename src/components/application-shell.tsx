...
export function ApplicationShell({ children }: { children: ReactNode }) {
  ...
  return <div className="app-canvas min-h-screen bg-background">
    ...
    <div className="relative z-10 mx-auto grid max-w-[1440px] gap-8 px-5 py-6 sm:px-8 md:grid-cols-[216px_minmax(0,1fr)] md:py-10"><aside className="hidden md:block"><nav className="no-scrollbar -mx-1 flex gap-1 overflow-x-auto px-1 pb-2 md:sticky md:top-24 md:flex-col md:overflow-visible" aria-label="Application">{links.map(([href, label, Icon]) => { const active = pathname === href; return <Link key={href} href={href} aria-current={active ? "page" : undefined} className={cn("flex shrink-0 items-center gap-3 rounded-xl px-3 py-2.5 text-sm transition", active ? "bg-ink text-white shadow-lg shadow-black/10" : "text-muted-foreground hover:bg-white hover:text-foreground")}><Icon className={cn("size-4", active ? "text-emerald-300" : "text-muted-foreground")}/>{label}</Link>; })}</nav></aside><main className="page-stage min-w-0 pb-24 md:pb-16">{children}</main></div>
    <nav className="mobile-tab-bar md:hidden" aria-label="Primary application navigation">{links.filter(([href]) => ["/dashboard", "/documents", "/medical", "/timeline", "/settings/privacy"].includes(href)).map(([href, label, Icon]) => { const active = pathname === href; return <Link key={href} href={href} aria-current={active ? "page" : undefined} className={cn("mobile-tab", active && "mobile-tab--active")}><Icon className="size-5"/><span>{label === "Overview" ? "Home" : label === "Life timeline" ? "Timeline" : label}</span></Link>; })}</nav>
  </div>;
}
