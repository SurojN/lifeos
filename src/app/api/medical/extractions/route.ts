...
export async function POST(request: Request): Promise<Response> {
  try {
    ...
    const service = new MedicalExtractionService(getDatabase(), createPrivateStorage(), getApplicationEncryption());
    return Response.json(await service.extract(user.id, await request.json()), { status: 201 });
  } catch (error) {
