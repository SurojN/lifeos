...
Current controls include verified Clerk sessions, signed Svix webhook verification, a persistent replay ledger, internal-user mapping, ownership-scoped queries, compound tenant foreign keys, authenticated application encryption, strict Zod input schemas, category-aware signed PUT MIME/size/checksum constraints, user-prefixed generated keys, private S3 API calls, quarantine, authenticated owner deletion, safe client errors, security headers, and server-side success/denial audits.
